...
const LayerDetail = ({
  ...
  const handleTable = async (layerDetail) => {
    try {
      ...
      // First check if the current layer exists in the map
      ...
      // Find table elements or create one if it doesn't exist
      let tables = document.querySelectorAll(".table-element");
      
      // If no table elements exist, create one in the table panel
      if (tables.length === 0) {
        ...
        if (tablePanel) {
          ...
          tableElement.style.height = "calc(100vh - 200px)";
          tablePanel.appendChild(tableElement);
          
          // Update tables collection
          tables = document.querySelectorAll(".table-element");
        }
      }
      ...
      if (tables.length > 0) {
        tables.forEach(async (element) => {
          ...
          // Rest of your existing code for creating tables...
        ...
        console.warn("Tidak dapat menemukan atau membuat elemen '.table-element'");
      }
      setLoading(false);
    } catch (error) {
