...
const Catalog = ({
  ...
  try {
    ...
    console.error("Error adding highlight graphic:", error);
  }
};
  ...
  const fetchGeometryAndAttributes = async (layer, mapPoint) => {
      console.log("Fetching geometry and attributes for:", layer.title);
      ...
      try {
        if (layer.type === "map-image") {
          ...
          const params = new URLSearchParams({
            ...
            sr: mapPoint.spatialReference.wkid,
            tolerance: 5, 
          ...
          if (data.results && data.results.length > 0) {
            ...
            return {
              ...
              layerTitle: result.layerName || layer.title,
              layerId: result.layerId,
              variable_dialog_alias: layer.getVariableDialogForSubLayer(result.layerId).alias
            };
          }
        } 
          ...
          console.log(`Feature query returned ${featureSet.features.length} features`);
  ...
  return (
   <Box
         ...
         display={"flex"}
       >
         {/* Perbaikan pada IconButton di Header */}
         <Box
           sx={{
             display: "flex",
             flexDirection: "row",
