...
function getLayerName(year) {
  ...
  for (const { start, end, base } of intervals) {
    if (year >= start && year <= end) {
      return `KAB_Delta_WT_${base}`;
    }
  }
  // kalau di luar interval, berarti 1:1 dengan tahun
  return `KAB_Delta_WT_${year}`;
}
...
for (let year = 1975; year <= 2024; year++) {
  ...
  akuifertertekan2Layers.push({
    id: `akuifertertekan2Layers-${year}`,
    name,
...
export { url, akuifertertekan2Layers };
